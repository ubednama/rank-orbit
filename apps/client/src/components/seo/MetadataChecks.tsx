...
export default function MetadataChecks({ data, loading }: { data: Metadata | undefined, loading?: boolean }) {
    if (loading) {
            ...
            <Card className="bg-white/40 dark:bg-white/5 backdrop-blur-md border-gray-200/50 dark:border-white/10">
                ...
                <CardContent className="space-y-5">
                    <div className="space-y-2">
                        <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Title Tag</span>
                    ...
                    <div>
                        <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 block">H1 Headings</span>
                        <div className="flex flex-wrap gap-2">
                            {(data.h1 || []).map((h: string, i: number) => (
                                <Badge key={i} variant="secondary" className="px-3 py-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700">{h}</Badge>
                            ))}
                            {(!data.h1 || data.h1.length === 0) && <span className="text-sm text-red-500 font-medium">No H1 tags found</span>}
                        </div>
                    </div>
                </CardContent>
            </Card>
